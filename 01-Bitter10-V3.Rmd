...
03272025: What questions need answered?  

- *condition*: does olfaction impact perception of bitterness?  
+ hyp: nose closed will have same effect as nose open condition  
+ bwtn subj design, compare p-values between conditions  
  + is the bitterness of each sample significantly different across the conditions? (independent t-test, bonferroni)
  + is the difference in sample intensity different across the conditions? (independent t-test, bonferroni)
  
- **addition**: is there a difference between what is added? (S-H, W-H, S-W)
+ hyp: milk will reduce bitterness more than water; skim might reduce more than whole (based on @keastModificationBitternessCaffeine2008; although @bennettUsingMilkFat2012 did not find milkfat difference) 
  + R-index (2-AFC + confidence)
  + t-test w/ bonferroni (intensity ratings)

- quantity: does how much is added to the samples impact perceived bitterness?
+ hyp: adding 17% milk will reduce bitterness more than 8%
  + paired t-test w/ bonferroni
...
Milk10S8O <- RIndexV2(df10opened %>% select(comparison_19_v_21_more_bitter, confidence_21) %>%
                        count(comparison_19_v_21_more_bitter, confidence_21))
```
...
Milk10S17O <- RIndexV2(df10opened %>% select(comparison_13_v_15_more_bitter, confidence_15) %>%
                        count(comparison_13_v_15_more_bitter, confidence_15))
```
...
Milk10ROpen <- tibble(
  ...
  Percent = rep(factor(c("8% Addition", "17% Addition"),
                   levels = c("8% Addition", "17% Addition")), each = 2),
  Condition = "Closed",
  Rindex = c(Milk10S8O$RIND_S, Milk10S8O$RIND_N, Milk10S17O$RIND_S, Milk10S17O$RIND_N),
  Critical = as.numeric(Rcriti(length(df10opened$participant), length(df10opened$participant), 0.05, 2)[2]),
  Signal = rep(c("Signal", "Noise"), 2),
  Sig = ifelse(abs(Rindex) >= Critical & Signal == "Signal", "#B22222",
               ifelse(abs(Rindex) >= Critical & Signal == "Noise", "white",
                      "#888888")),
...
study1OpenMilkRViz <- Milk10ROpen %>% 
  ...
  theme(panel.grid.major = element_blank(),
        ...
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank()) +
  geom_text(data = Milk10ROpen, aes(Sample, Rindex, label = Addition), vjust = 1) 
```
...
water10S8O <- RIndexV2(df10opened %>% select(comparison_19_v_21_more_bitter, confidence_21) %>% 
  group_by(comparison_19_v_21_more_bitter) %>% 
  mutate(confidence_21 = factor(confidence_21, levels = c(1,2,3))) %>% 
  count(confidence_21, .drop = F) %>% arrange(desc(comparison_19_v_21_more_bitter)))
  
water10W8O  <- RIndexV2(df10opened %>% select(comparison_22_v_24_more_bitter, confidence_24) %>% 
  group_by(comparison_22_v_24_more_bitter) %>% 
  mutate(confidence_24 = factor(confidence_24, levels = c(1,2,3))) %>% 
  count(confidence_24, .drop = F) %>% arrange(desc(comparison_22_v_24_more_bitter)))
...
water10S17O <- RIndexV2(df10opened %>% select(comparison_14_v_17_more_bitter, confidence_17) %>% 
  group_by(comparison_14_v_17_more_bitter) %>% 
  mutate(confidence_17 = factor(confidence_17, levels = c(1,2,3))) %>% 
  count(confidence_17, .drop = F) %>% arrange(desc(comparison_14_v_17_more_bitter)))

water10W17O <- RIndexV2(df10opened %>% select(comparison_16_v_18_more_bitter, confidence_18) %>% 
  group_by(comparison_16_v_18_more_bitter) %>% 
  mutate(confidence_18 = factor(confidence_18, levels = c(1,2,3))) %>% 
  count(confidence_18, .drop = F) %>% arrange(desc(comparison_16_v_18_more_bitter)))
...
Water10ROpened <- tibble( #from 02072025
  Sample = factor(c(rep(c("B18","B17"), 4))),
  Name = c("water10S8O", "water10S8O", "water10W8O", "water10W8O",
             "water10S17O", "water10S17O", "water10W17O", "water10W17O"),
  Percent = rep(factor(c("8% Addition", "17% Addition"),
                   levels = c("8% Addition", "17% Addition")), each = 4),
  Condition = "Opened",
  Rindex = c(water10S8O$RIND_S, water10S8O$RIND_N, water10W8O$RIND_S, water10W8O$RIND_N,
             water10S17O$RIND_S, water10S17O$RIND_N, water10W17O$RIND_S, water10W17O$RIND_N),
  Signal = rep(c("Signal", "Noise"), 4),
  Critical = as.numeric(Rcriti(length(df10closed$participant), length(df10closed$participant), 0.05, 1)[2]),
  Sig = ifelse(Rindex >= Critical & Signal == "Signal", "#B22222", "#888888"),
  Comparison = c(rep(c("A", "A", "B", "B"), 2)),
...
compareD <- Water10ROpened %>% filter(Sig %in% "#B22222")
...
study1openedWaterRViz <- Water10ROpened %>% 
  ...
  theme(panel.grid.major = element_blank(),
        ...
        strip.background.y = element_blank(),
        strip.text.y = element_blank()) +
  geom_text(data = Water10ROpened, aes(Comparison, Rindex, label = Addition),
            vjust = 1) 
...
## Condition
### Same samples, different conditions (and different people)
```{r}
# create long data frame
df10longjoin <- full_join(df10closed_ratings_long %>% mutate(Condition = c("closed")), df10opened_ratings_long %>% mutate(Condition = c( "opened"))) 

df10longjoin %>% #test for constant variance
  group_by(Sample) %>% 
  rstatix::levene_test(Intensity ~ Condition) #for each sample, p < 0.05

# test with equal var
t.test(df10closed$bitterness_13, df10opened$bitterness_13, alternative = "two.sided", paired = FALSE, var.equal = TRUE)
t.test(df10closed$bitterness_14, df10opened$bitterness_17, alternative = "two.sided", paired = FALSE, var.equal = TRUE)
t.test(df10closed$bitterness_16, df10opened$bitterness_18, alternative = "two.sided", paired = FALSE, var.equal = TRUE)
t.test(df10closed$bitterness_19, df10opened$bitterness_21, alternative = "two.sided", paired = FALSE, var.equal = TRUE)
t.test(df10closed$bitterness_20, df10opened$bitterness_23, alternative = "two.sided", paired = FALSE, var.equal = TRUE)
t.test(df10closed$bitterness_22, df10opened$bitterness_24, alternative = "two.sided", paired = FALSE, var.equal = TRUE)
```

There were no significant differences in the bitterness intensity ratings across the nose closed or nose opened condition.

### Testing Sample Deltas
```{r}
df10closed %>% 
  mutate(Delta1315 =c(.$bitterness_15 - .$bitterness_13),
         Delta1417 = c(.$bitterness_17 - .$bitterness_14),
         Delta1618 = c(.$bitterness_18 - .$bitterness_16),
         Delta1921 =c(.$bitterness_21 - .$bitterness_19),
         Delta2023 = c(.$bitterness_23 - .$bitterness_20),
         Delta2224 = c(.$bitterness_24 - .$bitterness_22),
         Condition = c("closed")
         ) %>% 
  select(participant, Condition, starts_with("Delta")) 
...
df10Delta <- full_join(df10closed %>% 
  mutate(Delta1315 =c(.$bitterness_15 - .$bitterness_13),
         Delta1417 = c(.$bitterness_17 - .$bitterness_14),
         Delta1618 = c(.$bitterness_18 - .$bitterness_16),
         Delta1921 =c(.$bitterness_21 - .$bitterness_19),
...
t.test(Delta1315 ~ Condition, alternative = "two.sided", var.equal = TRUE, data = df10Delta)
# skim milk found more bitter than whole milk in opened condition
t.test(Delta1417 ~ Condition, alternative = "two.sided", var.equal = TRUE, data = df10Delta)$p.value
t.test(Delta1618 ~ Condition, alternative = "two.sided", var.equal = TRUE, data = df10Delta)
# the opened condition had the higher delta

t.test(Delta1921 ~ Condition, alternative = "two.sided", var.equal = TRUE, data = df10Delta)
t.test(Delta2023 ~ Condition, alternative = "two.sided", var.equal = TRUE, data = df10Delta)
t.test(Delta2224 ~ Condition, alternative = "two.sided", var.equal = TRUE, data = df10Delta)
# the opened condition had the higher delta


df10Delta %>% 
  select(-participant) %>% 
  pivot_longer(-c(Condition), names_to = "Comparison", values_to = "Delta") %>% 
  ggplot(aes(x = Comparison, y = Delta, group = Condition, color = Condition)) + 
  geom_col()

DeltaP <- c(
  round(t.test(Delta1315 ~ Condition, alternative = "two.sided", var.equal = TRUE, data = df10Delta)
$p.value, 2),
  round(t.test(Delta1417 ~ Condition, alternative = "two.sided", var.equal = TRUE, data = df10Delta)
$p.value, 2),
  round(t.test(Delta1618 ~ Condition, alternative = "two.sided", var.equal = TRUE, data = df10Delta)
$p.value, 2),
  round(t.test(Delta1921 ~ Condition, alternative = "two.sided", var.equal = TRUE, data = df10Delta)
$p.value, 2),
  round(t.test(Delta2023 ~ Condition, alternative = "two.sided", var.equal = TRUE, data = df10Delta)
$p.value, 2),
  round(t.test(Delta2224 ~ Condition, alternative = "two.sided", var.equal = TRUE, data = df10Delta)
$p.value, 2)
)
...
summarySE(df10Delta %>% 
  select(-participant) %>% 
  pivot_longer(-c(Condition), names_to = "Comparison", values_to = "Delta"),
  measurevar = "Delta", groupvars = c("Condition", "Comparison")) %>% 
  select(Condition, Comparison, Delta, sd) %>% 
  ggplot(aes(x = Comparison, y = Delta, group = Condition, color = Condition, fill = Condition)) +
  geom_bar(position = "dodge", stat = "identity") +
  annotate(geom = "text", x = c(1:6), y = -.9, label = formatC(DeltaP, format = 'f', digits = 2)) +
  #use formatC to include trailing zeros
  scale_y_continuous(limits = c(-1, 1.25)) +
  annotate(geom = "text", x = 1, y = -0.75, label = "p-value") +
  geom_hline(yintercept = 0)

# 03272025 could use ggsignif to include lines in comparisons, but likely not necessary
```

There were no significant differences in the deltas between comparisons across the nose closed or nose opened conditions. With the exception of the 17% skim milk/whole milk comparison, the nose opened condition always had a larger delta than the nose closed condition. Only the 17% whole milk/water comparison had a nose open delta greater than 1 point (on a 10 point scale).
