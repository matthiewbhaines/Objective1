...
```{r sour wrangling, include=FALSE}
sourclosed_long <- sourclosed %>% pivot_longer(cols = S1:S12, names_to = "Sample", values_to = "Sourness")
#length(sourclosed$Participant) #30 participants
coffeelevels

sourclosed_long$SampleName <- rep(c("Skim17-5", "Whole17-5", "Skim17-5", "Water17-5", "Whole17-5", "Water17-5",
                                    "Skim17-10", "Whole17-10", "Skim17-10", "Water17-10", "Whole17-10", "Water17-10"), 30)
#length(sourclosed_long$Participant) #360 rows
sourclosed_long$Condition <- rep("Closed", 360)
##view(sourclosed_long)

souropen_long <- souropen %>% select(Participant, S1:S12) %>% 
  pivot_longer(cols = S1:S12, names_to = "Sample", values_to = "Sourness")
length(souropen$Participant) # 34 participants
souropen_long$SampleName <- rep(c("Skim17-5", "Whole17-5", "Skim17-5", "Water17-5", "Whole17-5", "Water17-5",
                                    "Skim17-10", "Whole17-10", "Skim17-10", "Water17-10", "Whole17-10", "Water17-10"), 34)
souropen_long$Condition <- rep("Open", length(souropen_long$Participant))
#view(souropen_long)

sourratings <- full_join(sourclosed_long, souropen_long)
#view(sourratings) #combined data
```

```{r ta wrangling 2, include=FALSE}
TASummary

TAMean <- TASummary %>% select(Sample, mL_NaOH, sd) %>% 
  filter(Sample %in% c("Skim17-5", "Whole17-5", "Skim17-5", "Water17-5", "Whole17-5", "Water17-5",
                                    "Skim17-10", "Whole17-10", "Skim17-10", "Water17-10", "Whole17-10", "Water17-10")) 
...
```{r sour rcorr closed, echo=TRUE}
sour_to_cor_closed <- sourTAjoined %>% filter(cond == "Closed") %>%  select(mL_NaOH, Intensity) %>% as.matrix()
(sour_rcorr_closed <- rcorr(sour_to_cor_closed, type = "pearson")) #p = 0.0208
```
```{r sour rcorr open, echo=TRUE}
sour_to_cor_opened <- sourTAjoined %>% filter(Condition == "Open") %>%  select(mL_NaOH, Sourness) %>% as.matrix()
(sour_rcorr_opened <- rcorr(sour_to_cor_opened, type = "pearson")) #p = 0.0426
```
```{r sour_v_NaOH wrangling, include=FALSE}
sourTAjoined

colnames(sourclosed_long)
sourClosedSummary <- summarySE(data = sourclosed_long, measurevar = "Sourness", groupvars = "SampleName") %>% select(SampleName, Sourness, sd, se)
(sourOpenSummary <- summarySE(data = souropen_long, measurevar = "Sourness", groupvars = "SampleName") %>% select(SampleName, Sourness, sd, se))

sourClosedSummary$Condition <- rep("Closed", length(sourClosedSummary$SampleName))
sourOpenSummary$Condition <- rep("Open", length(sourOpenSummary$SampleName))

sourClosedSummary
SourSummary <- full_join(sourClosedSummary, sourOpenSummary)
SourSummary

TAMean3 <- TAMean2 %>% rename(c("sd" = "TA.sd"))
SourSummary <- full_join(SourSummary, TAMean3)
SourSummary
```
```{r Sour_v_NaOH_Viz, echo=FALSE, fig.cap='Mean sourness intensity ratings plotted against average titratable acidity readings expressed in mL of NaOH. Vertical error bars represent SE of intensity ratings. Horizontal error bars represent SD of titratable acidity readings.'}
SourSummary %>% 
  ggplot(aes(x = mL_NaOH, y = Sourness, color = SampleName)) +
  facet_grid(~Condition) +
  theme_bw() +
  geom_errorbar(aes(x = mL_NaOH, ymin = Sourness-se, ymax = Sourness+se), linewidth = .5, width = .25,color = "grey") +
  geom_errorbarh(aes(xmin = mL_NaOH-TA.sd, xmax = mL_NaOH+TA.sd, y = Sourness), linewidth = .5, height = 0.25, color = "grey") +
  geom_point(size = 2) +
