...
- discrimination seen between when 8% skim milk and 8% whole milk was added to coffee (R Index = `r round(as.numeric(Milk10S8C$RIND_S), 2)`, p $\leq$ **0.05**)  
...
- no discrimination between 17% skim milk or 17% whole milk added to coffee (R Index = `r round(as.numeric(Milk10S17C$RIND_N), 2)`, p > 0.05)  

```{r study1closed r wrangling milk, include=FALSE}
Milk10R <- tibble(
  Sample = c("Milk10S8C", "Milk10S8C", "Milk10S17C", "Milk10S17C"),
...
study1closedMilkRViz <- Milk10R %>% 
  ...
  scale_x_discrete(labels = c("Skim", "Whole")) +
...
```
- no discrimination between the 17% skim milk and 17% water samples (R Index = `r signif(water10S17C$RIND_S, 2)`, p > 0.05)  
...
Water10R <- tibble(
  Sample = factor(c(rep(c("B17","B17", "B18", "B18"), 2))),
...
Water10R$Signal <- forcats::fct_relevel(Water10R$Signal, "Noise", "Signal")

compareB <- Water10R %>% filter(Sig %in% "#B22222")
Water10R_Ylabels <- c("Signal" = "Water","Noise" = "Milk" )
Water10R$Add <- c("Water", "Skim", "Water", "Whole", "Water", "Skim", "Water", "Whole") 
compareB.1 <- Water10R %>% filter(Signal == "Noise") %>% filter(Rindex == max(Rindex)) 

rcritical_10W <- Water10R %>% filter(Sample == "B17" & Percent == "8% Addition")

#study1closedWaterRViz <- 
Water10R %>% 
  ggplot(aes(x = Name, y = Rindex, fill = Sig)) +
  geom_col(width = 0.5, color = "black") +
  facet_grid(Signal~Percent, labeller = labeller(Signal = as_labeller(Water10R_Ylabels))) +
  scale_fill_manual(breaks = sig_levels, values = sig_levels) +
  ...
  geom_signif(data = compareB,
              ...
              tip_length = 0,
              annotation = c("< 0.05")) +
  geom_signif(data = compareB.1,
              y_position = 0.90, xmin = 0.8, xmax = 1.2,
              tip_length = 0,
              annotation = c("< 0.10")) +
  geom_text(data = rcritical_10,
            label = "R-crit", aes(y = 0.7, x = 0.6), size = 3) +
  theme(panel.grid.major = element_blank(),
        ...
        legend.position = "none") +
  geom_text(data = Water10R %>% filter(Signal %in% "Noise"), aes(Sample, Rindex, label = Add),
             vjust = 2)
...
```{r}
# sent to Chris on 02072025 for feedback
Water10R <- tibble(
...
compareB <- Water10R %>% filter(Sig %in% "#B22222")
Water10R_Ylabels <- c(A = "Skim Milk", B = "Whole Milk" )
Water10R$Add <- c("Water", "Skim", "Water", "Whole", "Water", "Skim", "Water", "Whole") 
compareB.1 <- Water10R %>% filter(Signal == "Noise") %>% filter(Rindex == max(Rindex)) 
...
Water10R %>% 
  ...
  geom_col(width = 0.5, color = "black") +
  facet_grid(Comparison~Percent, labeller = labeller(Comparison = as_labeller(Water10R_Ylabels))) +
  scale_fill_manual(breaks = sig_levels, values = sig_levels) +
  scale_y_continuous(expand = expansion(mult = c(0, .1)), limits = c(0, 1), 
                     breaks = c(0, .25, .5, .75, 1),
                     labels = c("0%", "25%", "50%", "75%", "100%")) +
  ...
  theme(panel.grid.major = element_blank(),
        ...
        strip.background.y = element_blank(),
        strip.text.y = element_blank()) +
  geom_text(data = Water10R, aes(Sample, Rindex, label = Add),
             vjust = 2)
```
...
# 02032025: basically, we want to see NS on the top, and significance on the bottom
study1closedWaterRViz
```
